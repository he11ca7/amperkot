    ...
    int noteCount)
{
  ...
  for (int thisNote = 0; thisNote < noteCount; ++thisNote)
    {
      if (checkClick(button))
        {
          digitalWrite(led, LOW);
          return;
        }

      digitalWrite(led, toggleLeds);
      ...
      int duration = 1200 / durations[thisNote];
      int octaveMul = 2; // Множитель высоты нот

      tone(buzzer, octaveMul * notes[thisNote], duration);
